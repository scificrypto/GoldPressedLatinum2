...
#else
    const char* pszModule = "goldpressedlatinum";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GoldPressedLatinum2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GoldPressedLatinum2
    // Mac: ~/Library/Application Support/GoldPressedLatinum2
    // Unix: ~/.goldpressedlatinum2
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "goldpressedlatinum.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "goldpressedlatinumd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong GoldPressedLatinum will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("GoldPressedLatinum"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
