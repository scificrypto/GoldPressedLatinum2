...
const std::string CLIENT_NAME("GoldPressedLatinum");
...
#    define GIT_COMMIT_DATE "12 Nov 2018"
#endif

#define BUILD_DESC_FROM_COMMIT(maj,min,rev,commit) \
    "gpl-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-g" commit

#define BUILD_DESC_FROM_UNKNOWN(maj,min,rev) \
    "gpl-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-unk"
