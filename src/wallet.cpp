...
bool CWallet::CreateCoinStake(uint256 &hashTx, uint32_t nOut, uint32_t nGenerationTime, uint32_t nBits, CTransaction &txNew, CKey& key)
{
    ...
    int64_t nValueIn = 0;
    CoinsSet setCoins;
    if (!SelectCoinsSimple(nBalance - nReserveBalance, MIN_TX_FEE, MAX_MONEY, nGenerationTime, nCoinbaseMaturity * 20, setCoins, nValueIn))
        return false;
    ...
    {
        int64_t nSplitThreshold = GetArg("-splitthreshold", nCombineThreshold * 2);
