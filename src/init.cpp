...
void Shutdown(void* parg)
{
    ...
    RenameThread("goldpressedlatinum-shutoff");
...
//        CTxDB().Close();
        ...
        Sleep(50);
        printf("goldpressedlatinum exited\n\n");
        fExit = true;
#ifndef QT_GUI
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("GoldPressedLatinum version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  goldpressedlatinumd [options]                     " + "\n" +
                  "  goldpressedlatinumd [options] <command> [params]  " + _("Send command to -server or goldpressedlatinumd") + "\n" +
                  "  goldpressedlatinumd [options] help                " + _("List commands") + "\n" +
                  "  goldpressedlatinumd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "goldpressedlatinum:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("GoldPressedLatinum"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("GoldPressedLatinum"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: goldpressedlatinum.conf)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 7790 or testnet: 17790)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 7795 or testnet: 17795)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("GoldPressedLatinum version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "GoldPressedLatinum server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("GoldPressedLatinum"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of GoldPressedLatinum") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart GoldPressedLatinum to complete") << "\n";
