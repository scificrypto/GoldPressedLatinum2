...
def confFile():
    ...
    if platform.system() == 'Windows':
        folder = os.path.join(os.path.join(os.environ['APPDATA'], 'goldpressedlatinum'))
    else:
        if platform.system() == 'Darwin':
            folder = os.path.expanduser('~/Library/Application Support/goldpressedlatinum/')
        else:
            folder = os.path.expanduser('~/.goldpressedlatinum')

    return os.path.join(folder, 'goldpressedlatinum.conf')

conf_path = confFile()
if not os.path.exists(conf_path):
    parser.error('''goldpressedlatinum configuration file not found. Manually enter your RPC password.\r\n'''
        ...
        '''rpcpassword=%x\r\n'''
        '''\r\n'''
        '''Keep that password secret! After creating the file, restart goldpressedlatinum.''' % (conf_path, random.randrange(2**128)))
